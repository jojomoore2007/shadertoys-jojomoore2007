...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for (int i = 1; i < 8; i++) {
        ...
        float cr = fnoise(vec3(rotate(uv,-dt)+(vec2(0.0,0.75)*dc),(ts*time)-dt),7)-edge;
        float cg = fnoise(vec3(uv+(vec2(sqrt(0.75),-0.75)*dc),(ts*time)),7)-edge;
        float cb = fnoise(vec3(rotate(uv,dt)+(vec2(-sqrt(0.75),-0.75)*dc),(ts*time)+dt),7)-edge;
